...
static void aes128_ecb_encrypt_old(uint8_t *plaintext, uint8_t *key,
                                   uint8_t *ciphertext) {
  ...
  /* aes.encrypt.old: opcode=0x6b funct3=010 (old separate encrypt module) */
  __asm__ volatile(".insn r 0x6b, 2, 0, x0, x0, x0");
...
int main(void) {
  ...
  uint8_t key[16] = {'c', 'e', 's', 'e', '4', '0', '4', '0',
                     'p', 'a', 's', 's', 'w', 'o', 'r', 'd'};
  /* Same expected output as main_encrypt.c — same algorithm, funct3=010 */
  uint8_t expected[16] = {0x14, 0x09, 0xA5, 0xFB, 0x1F, 0xF4,
                           0x4B, 0x71, 0xBE, 0xAA, 0x25, 0x2E,
                           0x0F, 0x08, 0xF9, 0xAA};
  uint8_t ciphertext[16] = {0};

  aes128_ecb_encrypt_old(plaintext, key, ciphertext);

  uintptr_t base = 0x0100000 + 0x2000;
  write_block(base + 0x30, expected);
  write_block(base + 0x40, ciphertext);
