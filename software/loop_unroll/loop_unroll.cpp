...
public:
  PreservedAnalyses run(Loop &loop, LoopAnalysisManager &analysis_manager,
                        LoopStandardAnalysisResults &analysis_res,
                        LPMUpdater &updater) {
    // since we only want to unroll our aes loop we need to make sure that our
    // loop is the right one so iterate over all instructions to check if we
    // have our aes32esmi instr
        ...
        if (auto *call_instr = dyn_cast<CallInst>(&instr)) {
          if (call_instr->isInlineAsm()) {
            ...
            if (asm_str.contains("aes32esmi") ||
                asm_str.contains("aes32dsmi")) {
              // found our loop
              UnrollLoopOptions unroll_options;
              unroll_options.Count = 9;
              unroll_options.Force = true;

              UnrollLoop(&loop, unroll_options, &analysis_res.LI,
                         &analysis_res.SE, &analysis_res.DT, &analysis_res.AC,
    ...
    // did not find the loop we want to unroll so return without modifying
    // anything
...
llvmGetPassPluginInfo() {
  return {LLVM_PLUGIN_API_VERSION, "AESLoopUnroll", LLVM_VERSION_STRING,
          [](PassBuilder &PB) {
