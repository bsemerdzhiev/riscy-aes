...
void mix_columns_inv(uint8_t *state) {
  ...
  for (int i = 0; i < 4; i++) {
    int idx = i * 4;
    temp[idx] = gf_mult(0x0e, state[idx]) ^ gf_mult(0x0b, state[idx + 1]) ^
                gf_mult(0x0d, state[idx + 2]) ^ gf_mult(0x09, state[idx + 3]);

    temp[idx + 1] = gf_mult(0x09, state[idx]) ^ gf_mult(0x0e, state[idx + 1]) ^
                    gf_mult(0x0b, state[idx + 2]) ^
                    gf_mult(0x0d, state[idx + 3]);

    temp[idx + 2] = gf_mult(0x0d, state[idx]) ^ gf_mult(0x09, state[idx + 1]) ^
                    gf_mult(0x0e, state[idx + 2]) ^
                    gf_mult(0x0b, state[idx + 3]);

    temp[idx + 3] = gf_mult(0x0b, state[idx]) ^ gf_mult(0x0d, state[idx + 1]) ^
                    gf_mult(0x09, state[idx + 2]) ^
                    gf_mult(0x0e, state[idx + 3]);
  }
  for (int i = 0; i < 16; i++) {
    state[i] = temp[i];
  }
...
void aes128_ecb_decrypt(uint8_t *plaintext, size_t len, uint8_t *key,
  ...
  for (size_t i = 0; i < len; i += 16) {
    aes128_decrypt_block(&plaintext[i], round_keys, &ciphertext[i]);
  }
}
